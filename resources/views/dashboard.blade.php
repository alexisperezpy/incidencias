    ...
    <div class="card">
        ...
        @if (auth()->user()->is_admin)
            <div class="card card-info mx-3 my-3">
                <div class="card-body">
                    <h4 class="card-title text-center text-secondary">Todas las Incidencias</h4>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody id="all-incidents">
                                @if(count($allIncidents) < 1)
                                    <tr>
                                        <td colspan="7" class="text-center">No existen Incidencias asingnadas</td>
                                    </tr>  
                                @else
                                    @foreach ($allIncidents as $incidente)                             
                                        <tr>
                                            <td class="text-center">
                                                <a href="/ver/{{ $incidente->id }}">{{ $incidente->id }}</a>
                                            </td>
                                            <td>{{ $incidente->category_name }}</td>

                                            @if($incidente->severity == "A")
                                                <td class="bg-danger text-center">{{ $incidente->severity_full }} </td>
                                            @else
                                                <td>{{ $incidente->severity_full }} </td>
                                            @endif
                                                <td>{{ $incidente->created_at }}</td>
                                                @if($incidente->state === 'Resuelto')
                                                    <td class="bg-success">{{ $incidente->state }}</td>
                                                @else
                                                    <td class="bg-warning">{{ $incidente->state }}</td>
                                                @endif
                                                <td>{{ $incidente->description_short }}</td>
                                        </tr>
                                    @endforeach
                                @endif
                            </tbody>
